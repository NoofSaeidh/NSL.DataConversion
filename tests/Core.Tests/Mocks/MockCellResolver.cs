...
{
    public class MockCellResolver : ICellResolver
    {
        public virtual ICell<T> Resolve<T>(T value)
        {
            return new MockCell<T>(value);
        }

        public virtual ICell ResolveObject(object value)
        {
            if (value == null) return new MockCell(value);
            return (ICell)Activator.CreateInstance(typeof(MockCell<>).MakeGenericType(value.GetType()), value);
        }
        ...
        ICell<T> ICellResolver.Resolve<T>(T value) => Resolve(value);

        ICell IResolver<object, ICell>.Resolve(object value) => Resolve(value);
        ...
        ICell IObjectResolver<ICell>.ResolveObject(object value) => Resolve(value);
    }
}